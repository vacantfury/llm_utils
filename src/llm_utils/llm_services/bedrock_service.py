...
``SlurmClusterService``. boto3 is sync-only, so each ``converse`` call runs in
...
# stop the whole run FAST with an actionable message. Temporary STS credentials
# in particular expire after hours and are not auto-refreshed by boto3.
_CREDENTIAL_MARKERS = (
